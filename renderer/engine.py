...
class RenderEngine:
    ...
    def render_multiprocess(self, scene, process_count, img_fileobj):
        ...
        # Split data for sharing with processes using a file implementation
        temp_dir = Path(tempfile.mkdtemp())
        temp_file_tmp1 = "render-part-{}.temp"
        processes = []
        try:
            rows_done = Value("i", 0)
            for hmin, hmax in ranges:
                # join directory with file name
                part_file = temp_dir / temp_file_tmp1.format(hmin)
                processes.append(Process(target=self.render, args=(
                    scene, hmin, hmax, part_file, rows_done)))
            # Start all processes
    ...
    def render(self, scene, hmin, hmax, part_file, rows_done):
        ...
        # Cast a ray through every pixels
        for j in range(hmin, hmax):
            ...
            if rows_done:
                with rows_done.get_lock():
                    rows_done.value += 1
                    print("{:3.0f}%".format(
                        float(rows_done.value) / float(height) * 100), end="\r")
    ...
    def color_at(self, obj_hit, hit_pos, normal, scene):
        ...
        for light in scene.lights:
            ...
            # Check if there is an object intersecting the to_light ray
            _, obj_hit = self.find_nearest(to_light, scene)
            # Unless if none, there is no shadow
            if obj_hit is None:
